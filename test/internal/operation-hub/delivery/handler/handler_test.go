...
func TestHandlerJsonError(t *testing.T) {
	...
	assert.Equal(t, err.(exceptions.HandlerError).InternalMessage, "Error while trying to parse the SQS message")
...
func createEvent() *events.SQSEvent {
	...
	snsEventMessage, _ := json.Marshal(events.SNSEvent{
		Records: []events.SNSEventRecord{
			{
				SNS: events.SNSEntity{
					Message: string(analysisMessage),
				},
			},
		},
	})
